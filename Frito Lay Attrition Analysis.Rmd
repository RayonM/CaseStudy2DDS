...
ggplot(data = employee_data, mapping = aes(x = OverTime, fill = Attrition ),position = "dodge")+
  geom_bar()
...
employee_data %>% count(Attrition,OverTime) %>%
  mutate(prop = n / sum(n)) %>%
  ggplot() +
  geom_tile(mapping = aes(x = Attrition, y = OverTime, fill = Attrition)) +
...
employee_data %>% count(Attrition,OverTime) %>%
  ggplot() +
  geom_tile(mapping = aes(x = Attrition, y = OverTime, fill = Attrition)) +
  labs(title = 'Distribution of Attrition within OverTime')
...
# employee_data %>% select(YearsAtCompany, YearsInCurrentRole) %>% 
#   ggpairs(mapping = aes(color = Attrition))
...
for(seed in 1:100)
{
  ...
  model_attrition_yesorno = naiveBayes(Train[,c("MonthlyIncome", "JobLevel", "OverTime")], factor(Train$Attrition, labels=c("No", "Yes")), laplace = 1)
...
xmaster_acc = numeric(100)
xmaster_sensitivity = numeric(100)
xmaster_specificivity = numeric(100)
xmaster_seed = numeric(100)
...
for(seed in 1:100)
{
  ...
  model_attrition_yesorno = naiveBayes(Train[,c("MonthlyIncome", "JobLevel", "OverTime")], Train$Attrition, laplace = 1)
  CM = confusionMatrix(table(predict(model_attrition_yesorno,Test[,c("MonthlyIncome", "JobLevel", "OverTime")],Test$Attrition)))
  
  xmaster_acc[seed] = CM$overall[1]
  xmaster_sensitivity[seed] = CM$byClass[1]
  xmaster_specificivity[seed] = CM$byClass[2]
  xmaster_seed[seed] = seed
}

mean(xmaster_seed)
mean(xmaster_acc)
mean(xmaster_sensitivity)
mean(xmaster_specificivity)
...
```{r}
  ...
  model_attrition_yesorno = naiveBayes(Train[,c(20,16,24)], factor(Train$Attrition, labels=c("No", "Yes")))
  table(predict(model_attrition_yesorno, Test[,c(20,16,24)], factor(Train$Attrition, labels=c("No", "Yes"))))
  CM = confusionMatrix(table(predict(model_attrition_yesorno,Test[,c(20,16,24)], factor(Train$Attrition, labels=c("No", "Yes")))))
...
CM = confusionMatrix(table(factor(Test$Attrition, labels = c("No", "Yes")), predict(model_attrition_yesorno,Test[,c("MonthlyIncome", "JobLevel", "OverTime")], type = "raw")))
...
comp_dataset$Attrition =  predict(model_attrition_yesorno,comp_dataset[,c("MonthlyIncome", "JobLevel", "OverTime")])
